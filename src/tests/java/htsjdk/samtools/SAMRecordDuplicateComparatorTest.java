...
public class SAMRecordDuplicateComparatorTest {
    ...
    private void assertEquals(List<Integer> expectedValues, final SAMRecordSetBuilder records, boolean fullCompare) {
        SAMRecord previous = null, current = null;
    ...
    private void assertEquals(int expected, final SAMRecordSetBuilder records, boolean fullCompare) {
        assertEquals(Arrays.asList(expected), records, fullCompare);
    }
    ...
    /***
     * Tests for comparing duplicate sets only, paired end reads.
     */
    @Test
    ...
    public void test() {
        ...
        records.addPair("READ0", 1, 10040, 10040, false, true, "76M", null, false, false, DEFAULT_BASE_QUALITY);
        records.addPair("READ1", 1, 10189, 10040, false, false, "41S35M", "65M11S", true, false, DEFAULT_BASE_QUALITY); // mapped OK
